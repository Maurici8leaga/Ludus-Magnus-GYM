...
routerComment.delete('/comment/:id/:comment_id', async (req, res) => {
    try {

        const videoId = await Videos.findById({_id: req.params.id});
        // de esta forma extraemos el comentario
        const commentario = videoId.comment.find(commentario => commentario.id === req.params.comment_id);
        ...
        const removeComment = videoId.comment.map(commentario => commentario.alumno.toString()).indexOf(req.user.id);
        ...
        videoId.comment.splice(removeComment, 1);
